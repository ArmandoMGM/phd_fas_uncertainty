...
class IQDClassifier():
    def __init__(self,  
        ...
        self.url_drive = 'https://drive.google.com/uc?id=11WDCzIA0gaMDJbV0p9glqfPvBJF5a33J'      
    ...
    def load_model(self):
        ...
        """
        if not os.path.exists(self.iqd_model_path): 
            print(f"File '{self.iqd_model_name}' does not exist. Downloading file from drive...")
            os.makedirs(os.path.dirname(self.iqd_model_path), exist_ok=True)

        try:
            gdown.download(self.url_drive, self.iqd_model_path, quiet=False) 
            print(f"File downloaded successfully to: {self.iqd_model_path}")
